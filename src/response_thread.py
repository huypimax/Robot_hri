...
class ResponseThread(QThread):
    ...
    def __init__(self, query):
        ...
        self.api_key = "AIzaSyCsnVbGzLouYNPXIJxnYdmQFa2BrRo1uqA"  # ← Đổi thành key riêng
        genai.configure(api_key=self.api_key)
        self.model = genai.GenerativeModel("gemini-2.0-flash")

        self.initial_context = [
            {"role": "user", "parts": [{"text": f"You are a helpful, concise virtual assistant named {ASSISTANT_NAME}, created by Fablab."}]},
            {"role": "model", "parts": [{"text": f"Understood. I'm {ASSISTANT_NAME}, created by Fablab."}]},
    ...
    def run(self):
        print("In response_thread")
        if self.isInterruptionRequested():
            print("🛑 ResponseThread interrupted before starting.")
            return
        ...
        if any(kw in self.query for kw in ["stop", "thank you", "bye"]):
            ...
            self.finished.emit(f"It is {current_time}.")
            return
            ...
            self.finished.emit(f"Today is {date_str}.")
            return
        ...
        if self.isInterruptionRequested():
            print("🛑 Interrupted before calling Gemini.")
            return

        if faq_answer:
            self.finished.emit(faq_answer)
            return

        # If not FAQ, use Gemini
        try:
            reply = asyncio.run(self.ask_gemini(self.query))
            if self.isInterruptionRequested():
                print("🛑 Interrupted after Gemini replied.")
                return
            self.last_reply = reply
            self.finished.emit(reply)
        except Exception as e:
            print("Gemini error:", e)
            self.finished.emit("Sorry, I have trouble getting an answer.")
    ...
    def check_faq(self, query: str):
        ...
        if any(kw in q for kw in [
            ...
            return ("Ho Chi Minh City University of Technology, also known as Bách Khoa, "
                    "is one of Vietnam’s top technical universities...")

        elif any(kw in q for kw in [
            "fablab", "innovation lab", "robotics lab", "fab lab"
        ]):
            return "Fablab is an innovation lab at Ho Chi Minh University of Technology..."

        elif any(kw in q for kw in [
            "who created you", "your creator", "who made you", "who built you"
        ]):
            return "I was created by a group of students from Fablab laboratory..."
        ...
        elif any(kw in q for kw in ["help", "what can you do", "what are your functions"]):
            return "I can answer questions, tell the time, and guide you..."

        else:
            return None

    async def ask_gemini(self, prompt):
        context = self.initial_context + [{"role": "user", "parts": [{"text": prompt}]}]
        try:
